import request from 'supertest';
import { describe, it, expect, vi, beforeEach } from 'vitest';

const mocks = vi.hoisted(() => ({
  mockBuildCheckoutPreview: vi.fn(),
}));

vi.mock('../../middleware/admin.middleware.ts', () => ({
  requireAdmin: (req: any, res: any, next: any) => {
    const authHeader = req.headers.authorization;

    if (!authHeader) {
      return res.status(401).json({ message: 'Missing token' });
    }

    req.user = { id: 1, role: 'customer' };
    next();
  },
}));

vi.mock('../../services/checkoutService/checkout.service', () => ({
  buildCheckoutPreview: mocks.mockBuildCheckoutPreview,
}));

import app from '../../app';

describe('POST /api/checkout/preview', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('returns 401 when authorization header is missing', async () => {
    const res = await request(app)
      .post('/api/checkout/preview')
      .send({
        items: [{ productId: 1, quantity: 2 }],
      });

    expect(res.status).toBe(401);
  });

  it('returns 400 when items are missing', async () => {
    const res = await request(app).post('/api/checkout/preview').set('Authorization', 'Bearer valid-token').send({});

    expect(res.status).toBe(400);
    expect(res.body).toEqual({
      message: 'Valid items are required',
    });
  });

  it('returns 400 when items are invalid', async () => {
    const res = await request(app)
      .post('/api/checkout/preview')
      .set('Authorization', 'Bearer valid-token')
      .send({
        items: [{ productId: 'abc', quantity: 0 }],
      });

    expect(res.status).toBe(400);
    expect(res.body).toEqual({
      message: 'Valid items are required',
    });
  });

  it('returns checkout preview when payload is valid', async () => {
    mocks.mockBuildCheckoutPreview.mockResolvedValue({
      customerGroupCode: 'retail',
      items: [
        {
          productId: 10,
          quantity: 2,
          original_unit_price: 100,
          unit_price: 90,
          discount_amount: 20,
          total_price: 180,
          applied_discount_id: 1,
          applied_discount_name: 'Spring Sale',
        },
      ],
      subtotal: 180,
      coupon: {
        id: 99,
        code: 'SAVE10',
        discountType: 'percent',
        discountValue: 10,
        discountAmount: 18,
      },
      grandTotal: 162,
    });

    const res = await request(app)
      .post('/api/checkout/preview')
      .set('Authorization', 'Bearer valid-token')
      .send({
        items: [{ productId: 10, quantity: 2 }],
        couponCode: 'SAVE10',
      });

    expect(res.status).toBe(200);

    expect(mocks.mockBuildCheckoutPreview).toHaveBeenCalledWith({
      items: [{ productId: 10, quantity: 2 }],
      userId: 1,
      couponCode: 'SAVE10',
    });

    expect(res.body).toEqual({
      customerGroupCode: 'retail',
      items: [
        {
          productId: 10,
          quantity: 2,
          original_unit_price: 100,
          unit_price: 90,
          discount_amount: 20,
          total_price: 180,
          applied_discount_id: 1,
          applied_discount_name: 'Spring Sale',
        },
      ],
      subtotal: 180,
      coupon: {
        id: 99,
        code: 'SAVE10',
        discountType: 'percent',
        discountValue: 10,
        discountAmount: 18,
      },
      grandTotal: 162,
    });
  });
});
